...
-- 5 - Faça uma query que retorna a soma total do estoque de todos os medicamentos das 
--     categorias blue e black na tabela farmacia.
...
-- 9 - Faça uma query que retorna uma única coluna com a junção do nome do medicamento e a categoria entre 
--     parenteses de todos os registros em que a categoria não seja nula da tabela farmacia. Ex.: Endocet (green).
...
--      cadastrado na tabela usuarios. O tempo precisa ser uma coluna que retorne a quantidade de 
--      anos, meses, dias, horas, minutos e segundos em um objeto. 
--      Ex.: {"years":2,"months":2,"days":19,"hours":8,"minutes":22,"seconds":19}.

SELECT nome, idade, email, AGE(NOW(), cadastro::TIMESTAMP) FROM usuarios WHERE idade < 18;
...
SELECT nome, idade, email, AGE(NOW()::DATE, cadastro::DATE) FROM usuarios WHERE idade >= 60;
...
-- 16 - Faça uma query que retorna as três categorias e a soma do estoque de todos os 
--      medicamentos de cada categoria, na tabela farmacia.
